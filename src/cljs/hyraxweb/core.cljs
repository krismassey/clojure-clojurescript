...
(defn widget-box [app owner opts]
      (reify
        ...
        (render [_]
                (dom/h1 nil "Widgets")
                (om/build widget-list (app widgets)))))
...
;(om/root om-app app-state (.getElementById js/document "content"))
;(om/root (fn [app owner] om-app) app-state {:target (. js/document (getElementById "content"))})
